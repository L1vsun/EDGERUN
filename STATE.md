...
## Page rhythm + README (2026-09-21)

The front page had four unrelated blocks sharing one screen. Fixed by giving each its own
band: the hero is now `min-height: 100vh - header` with the content centred and a scroll cue
at the bottom naming what comes next ("Next - install it in two minutes"), and every major
section (`.install`, `.feed`, `.cortex`, `.council`) gets 104px of top padding and a hairline
above it. Type went up across the board (hero body 15 -> 18px, step copy 13.5 -> 15px, surface
copy 12.5 -> 14px, flag key rebuilt as a list instead of a run-together paragraph), and the
densest paragraphs were cut roughly in half. The four-box "surfaces" row in the hero collapsed
to one line, since the install section already covers each surface properly.

Two real bugs found while doing it: `.step b` was styling *every* `<b>` inside a step as a
block, so inline bolds in step 02 broke onto their own lines; and the header does not wrap, so
the added social icons pushed `.top-actions` 16px past the viewport at 390px (the secondary
button now hides below 760px). Verified: no horizontal overflow at 390px, 0 long dashes.

`README.md` rewritten around the extension - it still led with the `edgerun scan` CLI and never
